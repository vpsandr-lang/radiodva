"""RADIO DVA AI — CD-quality mixer. Concatenates voice → music → voice.
High bitrate output with natural room ambiance.
"""
import os, subprocess, hashlib, random
from pathlib import Path

class AudioMixer:
    def __init__(self):
        self.work = "/tmp/radio/mixer"
        os.makedirs(self.work, exist_ok=True)

    def get_music(self, style):
        return None

    def create_broadcast_segment(self, music_wav, voice_intro_wav=None, voice_outro_wav=None):
        h = hashlib.md5(f"{music_wav}:{voice_intro_wav}:{voice_outro_wav}:{random.random()}".encode()).hexdigest()[:10]
        out = f"/tmp/radio/segments/seg_{h}"
        os.makedirs("/tmp/radio/segments", exist_ok=True)

        # Collect valid inputs
        inputs = []
        labels = []
        for fname, label in [(voice_intro_wav, "intro"), (music_wav, "music"), (voice_outro_wav, "outro")]:
            if fname and os.path.exists(fname) and os.path.getsize(fname) > 100:
                inputs.append(fname)
                labels.append(label)

        if not inputs:
            subprocess.run([
                "ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=44100:cl=stereo",
                "-t", "10", "-acodec", "pcm_s16le", f"{out}.wav"
            ], capture_output=True, timeout=30)
            return f"{out}.wav"

        # Single input - copy with CD quality
        if len(inputs) == 1:
            try:
                subprocess.run([
                    "ffmpeg", "-y", "-i", inputs[0],
                    "-acodec", "pcm_s16le", "-ac", "2", "-ar", "44100",
                    f"{out}.wav"
                ], capture_output=True, timeout=30)
                if os.path.exists(f"{out}.wav") and os.path.getsize(f"{out}.wav") > 100:
                    return f"{out}.wav"
            except: pass
            return None

        # Multiple inputs - concatenate sequentially
        filter_parts = "".join(f"[{i}:a]" for i in range(len(inputs)))
        concat_filter = f"{filter_parts}concat=n={len(inputs)}:v=0:a=1[out]"

        try:
            cmd = ["ffmpeg", "-y"]
            for inp in inputs:
                cmd.extend(["-i", inp])
            cmd.extend([
                "-filter_complex", concat_filter,
                "-map", "[out]",
                "-acodec", "pcm_s16le", "-ac", "2", "-ar", "44100",
                f"{out}.wav"
            ])
            subprocess.run(cmd, capture_output=True, timeout=120)
        except subprocess.TimeoutExpired:
            pass

        if os.path.exists(f"{out}.wav") and os.path.getsize(f"{out}.wav") > 100:
            return f"{out}.wav"
        return None

    def wav_to_mp3(self, wav_path):
        """Convert WAV to MP3 at high quality (128kbps)."""
        if not wav_path or not os.path.exists(wav_path):
            return None
        mp3 = wav_path.replace('.wav', '.mp3')
        try:
            subprocess.run([
                'lame', '--quiet', '-h', '-b', '96', wav_path, mp3
            ], capture_output=True, timeout=60)
            if os.path.exists(mp3) and os.path.getsize(mp3) > 100:
                try: os.remove(wav_path)
                except: pass
                return mp3
        except: pass
        return wav_path
